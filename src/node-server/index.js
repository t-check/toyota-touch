...
app.get('/files/:drive/:folderName', function(req, res){
    ...
    try{
        fs.readdir('/media/pi/' + req.params.drive + '/' + req.params.folderName, function(err, files){
            if (err){
                res.writeHead(200, {'Content-Type': 'application/javascript'});
...
app.get('/folders/:drive', function(req, res){
    ...
    try{
        ...
        var directories = getDirectories('/media/pi/'+ req.params.drive);
...
app.get('/get-default-drive', function(req, res){
    ...
    try{
        ...
        var directories = getDirectories('/media/pi');
        ...
        for(var i=0; i<disks.length; i++){
            try{
                var dirs = getDirectories('/media/pi/' + disks[i].name);
...
app.get('/media-file/:drive/*', function(req, res){
    ...
    res.sendfile(path, {root: '/media/pi/'+ drive});
});
